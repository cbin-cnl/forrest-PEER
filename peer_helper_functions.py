...
def apply_eyemask(image, eyemask_path):
    img = nib.load(image)
    data = img.get_fdata()
    eye_mask = nib.load(eyemask_path).get_fdata()
    arr = np.zeros((data.shape[3], np.sum(eye_mask==1)))
    for i in range(data.shape[3]):
        arr[i, :] = data[:, :, :, i][eye_mask==1]
    return arr
...
def make_targets(_calibration_points_removed, _stimulus_path, monitor_width, monitor_height):
